...
// Get average rating for a podcast (must be before /:userId/:podcastId to avoid conflict)
...
// Get all ratings (for analytics, must be before /:userId/:podcastId)
...
app.delete("/make-server-055f6039/feedback/delete/:feedbackId", async (c) => {
  try {
    ...
    const filtered = feedback.filter((f: any) => f.id !== feedbackId);

    if (filtered.length === feedback.length) {
      return c.json({ success: false, message: "Отзыв не найден" }, 404);
    }

    await kv.set("feedback", JSON.stringify(filtered));
    return c.json({ success: true, message: "Отзыв удалён" });
  } catch (error) {
...
app.post("/make-server-055f6039/podcast-reviews/add", async (c) => {
  try {
    const { userId, userName, podcastId, reviewText } = await c.req.json();

    if (!userId || !userName || !podcastId || !reviewText) {
      return c.json({ success: false, message: "Все поля обязательны" }, 400);
    }
...
// Check if user has reviewed a podcast (must be before /:podcastId to avoid route conflict)
...
// Delete a podcast review (admin only)
app.delete("/make-server-055f6039/podcast-reviews/delete/:reviewId", async (c) => {
  try {
    ...
    await kv.set("podcast_reviews", JSON.stringify(filtered));
    return c.json({ success: true, message: "Отзыв удалён" });
  } catch (error) {
    console.log(`Delete podcast review error: ${error}`);
    return c.json({ success: false, message: `Ошибка удаления отзыва: ${error}` }, 500);
  }
});
