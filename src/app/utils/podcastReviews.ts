...
// Удалить отзыв к подкасту (только для администратора)
export const deletePodcastReview = async (reviewId: string): Promise<{ success: boolean; message: string }> => {
  try {
    const response = await fetch(`${API_BASE_URL}/podcast-reviews/delete/${reviewId}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${publicAnonKey}`,
      },
    ...
    console.log(`Delete podcast review API error: ${error}`);
