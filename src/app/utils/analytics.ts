...
import { PODCASTS_LIST } from "./podcastsList";
...
export const getComparisonChartData = async () => {
  ...
  return podcastAnalytics.map((analytics, index) => ({
    name: `#${index + 1} ` + (podcasts[index].title.length > 15 ? podcasts[index].title.substring(0, 12) + "..." : podcasts[index].title),
